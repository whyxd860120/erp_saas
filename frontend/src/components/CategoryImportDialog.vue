...
const props = defineProps<{
  ...
  formatTips: string[]
  importFn: (data: any) => Promise<{ success: boolean; message?: string }>
}>()
...
function downloadTemplate(type: 'excel' | 'csv') {
  ...
  const example = props.columns.map(c => {
    ...
    if (c.prop === 'parentName') return '上级分类'
    if (c.prop === 'sortOrder') return '0'
    return ''
  })
...
async function confirmImport() {
  ...
  try {
    for (let i = 0; i < validData.length; i++) {
      ...
      importProgress.value.currentItem = row.name || `第${i + 1}条`
