      ...
      <template #header>
        <div class="card-header">
          ...
          <div class="header-actions">
            <el-button v-if="hasPermission('supplier:create')" type="primary" plain @click="handleAddRootCategory">
              <el-icon><FolderAdd /></el-icon>
              新增供应商分类
            </el-button>
            <el-button v-if="hasPermission('supplier:create')" type="primary" plain :disabled="!selectedCategoryId" @click="handleAddChildCategory">
              <el-icon><FolderAdd /></el-icon>
              新增子分类
            </el-button>
            <el-button v-if="hasPermission('supplier:create')" type="primary" :disabled="!selectedCategoryId" @click="handleCreateSupplier">
            ...
            <el-dropdown v-if="hasPermission('supplier:create')" split-button type="success" plain @click="handleImport">
            ...
            <el-dropdown v-if="hasPermission('supplier:read')" split-button type="info" plain @click="handleExport">
      ...
      <div class="supplier-layout">
        ...
        <div class="category-panel">
          ...
          <div class="category-actions" v-if="selectedCategoryId">
            <el-button v-if="hasPermission('supplier:update')" link type="primary" @click="handleEditCategory">编辑分类</el-button>
            <el-button v-if="hasPermission('supplier:delete')" link type="danger" @click="handleDeleteCategory">删除分类</el-button>
          </div>
        </div>
...
import { Plus, Folder, FolderAdd, OfficeBuilding, Refresh, Upload, Download, View, Hide } from '@element-plus/icons-vue'
import {
...
const displaySuppliers = computed(() => {
  ...
  function extractSuppliers(items: any[]) {
    items.forEach(item => {
      if (item.nodeType === 'supplier') {
        result.push(item)
      } else if (item.children && item.children.length > 0) {
        extractSuppliers(item.children)
      }
    })
  }
  
  extractSuppliers(displayTreeData.value)
  return result
})
...
function buildDisplayTree(categories: CategoryNode[], suppliers: SupplierItem[]) {
  // 创建供应商映射，提升查找性能
  const supplierMap = new Map<string, SupplierItem[]>()
  const uncategorizedSuppliers: SupplierItem[] = []

  suppliers.forEach(supplier => {
    if (supplier.categoryId) {
      if (!supplierMap.has(supplier.categoryId)) {
        supplierMap.set(supplier.categoryId, [])
      }
      supplierMap.get(supplier.categoryId)!.push(supplier)
    } else {
      uncategorizedSuppliers.push(supplier)
    }
  })

  const mapSupplier = (s: SupplierItem) => ({ ...s, nodeType: 'supplier' })

  const walk = (cats: CategoryNode[]): any[] => {
    return cats.map(cat => {
      const subCategories = cat.children?.length ? walk(cat.children) : []
      const catSuppliers = (supplierMap.get(cat.id) || []).map(mapSupplier)
      return {
        id: `category-${cat.id}`,
        nodeType: 'category',
        name: cat.name,
        status: cat.status,
        categoryId: cat.id,
        hasChildren: subCategories.length > 0 || catSuppliers.length > 0,
        children: [...subCategories, ...catSuppliers]
      }
    })
  }
  ...
  if (uncategorizedSuppliers.length) {
    tree.push({
      id: 'category-uncategorized',
      nodeType: 'category',
      name: '未分类',
      categoryId: null,
      hasChildren: uncategorizedSuppliers.length > 0,
      children: uncategorizedSuppliers.map(mapSupplier)
    })
  }
...
function rebuildDisplayTree() {
  const subtree = selectedCategoryId.value
    ? (findCategoryNode(categoryTree.value, selectedCategoryId.value) ? [findCategoryNode(categoryTree.value, selectedCategoryId.value)!] : categoryTree.value)
    : categoryTree.value

  const categoryIds = collectCategoryIds(subtree)
  let suppliers = allSuppliers.value

  // 如果有搜索关键词，搜索不受分类限制
  if (searchForm.keyword.trim()) {
    const k = searchForm.keyword.trim().toLowerCase()
    suppliers = allSuppliers.value.filter(s =>
    ...
    // 没有搜索关键词时，按分类过滤
    suppliers = suppliers.filter(s => !s.categoryId || categoryIds.includes(s.categoryId!))
  }

  displayTreeData.value = buildDisplayTree(subtree, suppliers)
}
...
async function loadData() {
  try {
    ...
    const [catRes, supRes] = await Promise.all([
      getSupplierCategoryTree(),
      getSuppliers({ page: 1, limit: 2000, ...(showInactive.value ? {} : { status: 'active' }) })
    ])

    if (catRes.success) categoryTree.value = catRes.data || []
    if (supRes.success) allSuppliers.value = supRes.data.items || []
    rebuildDisplayTree()
  } catch (e) {
...
function handleTableRowClick(row: any) {
  if (row.nodeType === 'category' && row.categoryId) {
    selectedCategoryId.value = row.categoryId
    categoryTreeRef.value?.setCurrentKey(row.categoryId)
    rebuildDisplayTree()
  }
}
...
function handleAddChildCategoryFromRow(row: any) {
...
function handleEditCategoryFromRow(row: any) {
  if (!row.categoryId) return
  selectedCategoryId.value = row.categoryId
  handleEditCategory()
}
...
function handleCreateSupplier() {
  ...
  resetSupplierForm()
  if (selectedCategoryId.value) supplierForm.categoryId = selectedCategoryId.value
...
async function handleImportSubmit(data: any[]) {
  return await importSuppliers(data)
}
...
function handleExport() {
  ...
  const headers = ['供应商编码', '供应商名称', '联系人', '电话', '邮箱', '地址', '分类', '状态']
